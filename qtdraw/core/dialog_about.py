...
class AboutDialog(QDialog):
    ...
    def create_about_panel(self, parent):
        ...
        label_qtdraw = Label(parent, text="QtDraw", bold=True)
        label_copyright = Label(parent, text=self.pvw.copyright)
        label_python = Label(parent, text=f"\n{indent}Python: Ver. {python_ver}")
        label_pyvista = Label(parent, text=f"{indent}PyVista: Ver. {pyvista_ver}")
        label_pvqt = Label(parent, text=f"{indent}PyVistaQt: Ver. {pyvistaqt_ver}")
        label_vtk = Label(parent, text=f"{indent}VTK: Ver. {vtk_ver}")
        label_qt = Label(parent, text=f"{indent}PySide6: Ver. {pyside6_ver}")
        label_numpy = Label(parent, text=f"{indent}NumPy: Ver. {numpy_ver}")
        label_sympy = Label(parent, text=f"{indent}SymPy: Ver. {sympy_ver}")
        label_matplotlib = Label(parent, text=f"{indent}Matplotlib: Ver. {matplot_ver}")
        multipie = check_multipie()
        if multipie:
            from multipie import __version__ as multipie_ver

            label_multipie = Label(parent, text=f"{indent}MultiPie: Ver. {multipie_ver}")
