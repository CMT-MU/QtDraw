"""
Test for widgets.

This module provides a test for widgets.
"""

from PySide6.QtWidgets import QWidget

from qtdraw.sandbox.qt_event_util import get_qt_application
from qtdraw.sandbox.custom_widget import (
    Layout,
    Label,
    MathWidget,
    ColorSelector,
    Button,
    Combo,
    Spin,
    DSpin,
    Check,
    LineEdit,
    Editor,
)


# ==================================================
def test_widget():
    app = get_qt_application()

    bold = False

    label1 = Label(text="Normal Label")
    label2 = Label(text="Bold Label", bold=True)
    label3 = Label(text="Red Label", color="red")
    label4 = Label(text="Large Blue", color="blue", size=18)
    latex = r"\int_0^\infty e^{-x^2}\,dx = \frac{\sqrt{\pi}}{2}"
    label5 = MathWidget(text=latex, color="green")
    label6 = MathWidget(text="xyz", color="green")
    label9 = ColorSelector(current="strawberry", color_type="color", bold=bold)
    label9.currentTextChanged.connect(lambda c: print(c))
    label10 = ColorSelector(current="coolwarm", color_type="color_both", bold=bold)
    label10.currentTextChanged.connect(lambda c: print(c))
    label12 = Button(text="button", toggle=True, bold=bold)
    label12.clicked.connect(lambda c: print(c))
    label13 = Combo(item=["a", "b", "c"], bold=bold)
    label13.currentTextChanged.connect(lambda c: print(c))
    label14 = Check(text="check", bold=bold)
    label14.toggled.connect(lambda c: print(c))
    label15 = Spin(minimum=0, maximum=5, bold=bold)
    label15.valueChanged.connect(lambda c: print(c))
    label16 = DSpin(minimum=1.0, maximum=2.0, step=0.1, bold=bold)
    label16.valueChanged.connect(lambda c: print(c))

    edit1 = LineEdit(text="text", bold=bold)
    edit1.set_read_only(True)
    edit1.returnPressed.connect(lambda: print("display =", edit1.text(), "raw =", edit1.raw_text()))
    edit2 = LineEdit(text="0", validator=("int", {"min": -1, "max": 3}), bold=bold)
    edit2.returnPressed.connect(lambda: print("display =", edit2.text(), "raw =", edit2.raw_text()))
    edit3 = LineEdit(text="0.1", validator=("float", {"min": -1, "max": 3, "digit": 4}), bold=bold)
    edit3.returnPressed.connect(lambda: print("display =", edit3.text(), "raw =", edit3.raw_text()))
    edit4 = LineEdit(text="sqrt(3)", validator=("list_float", {"digit": 4}), bold=bold)
    edit4.returnPressed.connect(lambda: print("display =", edit4.text(), "raw =", edit4.raw_text()))
    edit5 = LineEdit(text="[1,2,3]", validator=("list_int", {"shape": (3,)}), bold=bold)
    edit5.returnPressed.connect(lambda: print("display =", edit5.text(), "raw =", edit5.raw_text()))
    edit6 = LineEdit(text="[1.0,2,3]", validator=("list_float", {"shape": (3,), "var": [""], "digit": 4}), bold=bold)
    edit6.returnPressed.connect(lambda: print("display =", edit6.text(), "raw =", edit6.raw_text()))

    edit7 = Editor(text="3z**2-r**2", bold=bold)
    edit7.returnPressed.connect(lambda: print("display =", edit7.text(), "raw =", edit7._editor.raw_text()))
    edit8 = Editor(text="3z**2-r**2", validator=("math", {}), bold=bold)
    edit8.returnPressed.connect(lambda: print("display =", edit8.text(), "raw =", edit8._editor.raw_text()))

    widget = QWidget()
    widget.resize(800, 100)
    layout = Layout(widget)
    layout.setContentsMargins(10, 10, 10, 10)
    layout.setHorizontalSpacing(10)
    layout.setVerticalSpacing(10)

    obj = [
        label1,
        label2,
        label3,
        label4,
        label5,
        label6,
        label9,
        label10,
        label12,
        label13,
        label14,
        label15,
        label16,
        edit1,
        edit2,
        edit3,
        edit4,
        edit5,
        edit6,
        edit7,
        edit8,
    ]

    for i, o in enumerate(obj):
        layout.addWidget(o, i // 4, i % 4)

    widget.show()

    app.exec()


if __name__ == "__main__":
    test_widget()
